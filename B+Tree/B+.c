...
#define MIN_DEGREE 3 //grau minimo de valores

typedef struct Node {
    int* keys; //vetor de chaves
    int t; //grau minimo de chaves
    struct Node** children; //vetord de ponteiros para os filhos
    int n; //numero de chaves no nodo
    bool leaf; //determina se e folha ou nao
    struct Node* next; //ponteiro para a proxima folha
} Node;

typedef struct BTree {
    Node* root; //ponteiro para a rai
    int t; //grau minimo
} BTree;
...
// Percorre a arvore e printa em ordem crescente
...
// Percorre a arvore procurando um valor
...
// Divide o filho caso o nodo esteja cheio
...
// Insere em um nodo o qual nao esta cheio
...
// Quando splitado insere uma chave na b+ tree
...
void procure(bool found, int key){
    if (found) {
